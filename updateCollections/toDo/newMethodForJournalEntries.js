...
dbObject.once('open', function callback() {
    ...
    WTrack.aggregate([{
            ...
            result.forEach(function (element) {
                ...
                datesArray.forEach(function (date) {
                    Model.remove({
                        ...
                        holiday = _.find(holidays, function (holiday) {
                            return moment(holiday.date).isSame(moment(date), 'year month day')
                        });
                        ...
                        for (var i = transferLength - 1; i >= 0; i--) {
                            ...
                            if (moment(date).isBefore(transferObj.date)) {
        ...
        var dataFinder = function (createdDateObject, wfCb) {
            ...
            async.parallel([vacationFinder, holidaysFinder], function (err, result) {
                if (err){
                    return wfCb(err);
                }

                wfCb(null, {createdDateObject: createdDateObject, vacations: result[0], holidays: result[1]});
            });
        };
        ...
        var createIdle = function (createdDateObject, empResult, wfCb) {
            ...
            async.each(newDates, function (dateKey, asyncCb) {
                ...
                var objectForDay = createdDateObject[dateKey] || {};
