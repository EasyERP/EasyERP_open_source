...
var Salary = function (models) {
    ...
    this.create = function (req, res, next) {
        ...
        salary.save(function (err, _quotation) {
            if (err) {
                return next(err);
            }
    ...
    function updateOnlySelectedFields(req, res, next, id, data) {
        ...
        Salary.findByIdAndUpdate(id, {$set: data}, function (err, salary) {
            if (err) {
                next(err);
            } else {
                res.status(200).send({success: 'Salary updated', result: salary});
            }
        });
    ...
    this.totalCollectionLength = function (req, res, next) {
        ...
        employeeSearcher = function (waterfallCallback) {
            models.get(req.session.lastDb, "Employee", EmployeeSchema).aggregate(
                {
                    $match: {
